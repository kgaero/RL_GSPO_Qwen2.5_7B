...
\begin{abstract}
We study a staged metric-gated GRPO fine-tuning pipeline for visual numeric reasoning. The method combines a phase-wise curriculum over filtered MathVista subsets, a strict \texttt{<REASONING>} / \texttt{<SOLUTION>} response contract, metric-gated reward adaptation based on checkpoint evaluation, and alias-based checkpoint selection for non-monotonic training trajectories. In resource-constrained runs, early small-scale phases stabilize parseability and output structure, and subsequent larger-split continuation improves correctness. Under the evaluation protocol used for continuation, the best Phase C and Phase D checkpoints reach exact-match and tolerance accuracy of 0.75, compared with 0.375 for the baseline, while maintaining parseability 1.0 and zero truncation on the monitored subset. These results suggest that curriculum staging, metric-gated reward control, and checkpoint-aware continuation can improve the stability of RL fine-tuning for multimodal numeric question answering.
