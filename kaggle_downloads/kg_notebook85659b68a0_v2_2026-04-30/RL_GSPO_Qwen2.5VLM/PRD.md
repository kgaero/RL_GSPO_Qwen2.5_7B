...
This PRD requires reusing the existing repository implementation. It is not a request to rewrite the training pipeline from scratch.
...
The implementation must preserve and reuse the existing code paths wherever possible:

```text
Training entrypoint: rl_gspo_qwen2_5vlm_test3.py
Evaluation entrypoint: rl_gspo_qwen2_5vlm_eval.py
Core package: staged_rl/
```

Expected changes should be limited to:

- Kaggle notebook metadata/input wiring
- checkpoint target manifests
- final re-evaluation orchestration
- report/table generation
- small compatibility fixes if required by Kaggle runtime

The following should not be rewritten unless a concrete blocker is found:

- stage definitions
- phase definitions
- reward functions
- metric-gated controller
- checkpoint artifact writing
- resume and warm-start mechanics
- model loading and LoRA attachment flow
...
## 6.1 Required Kaggle Input Datasets

All KG Kaggle notebooks must attach the local model-cache dataset before execution:

```text
kgaero/qwen2-5-vl-7b-unsloth-bnb4-cache-hf
```

The notebook must resolve the mounted model path under `/kaggle/input/...` and pass it to the training or evaluation runner through:

```text
--base-model-path /kaggle/input/.../resolved-model-root
```

The training code does not attach Kaggle datasets by itself. Dataset attachment is controlled by Kaggle notebook/kernel metadata. The runner only consumes the mounted path after Kaggle has attached the dataset.
