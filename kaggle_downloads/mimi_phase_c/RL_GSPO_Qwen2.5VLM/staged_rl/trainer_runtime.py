...
def _candidate_hf_cache_roots() -> list[Path]:
    """Return Hugging Face cache roots in priority order."""
...
def _resolve_local_model_snapshot(model_name_or_path: str) -> Optional[Path]:
    """Resolve a Hub repo id to a cached local snapshot if one already exists."""
    ...
    repo_cache_slug = "models--" + model_name_or_path.replace("/", "--")
    for cache_root in _candidate_hf_cache_roots():
        snapshots_dir = cache_root / repo_cache_slug / "snapshots"
        ...
        for snapshot in snapshots:
            if (snapshot / "config.json").exists() or (snapshot / "adapter_config.json").exists():
                return snapshot
        return snapshots[0]
    ...
    *,
    ...
    if adapter_model_path.exists():
        try:
            ...
            LOGGER.info(
                "Warm-started adapter from %s using direct safetensors state-dict load. "
                "Missing=%s Unexpected=%s",
                checkpoint_dir,
                sorted(getattr(load_result, "missing_keys", [])),
                sorted(getattr(load_result, "unexpected_keys", [])),
            )
            return
        except Exception as exc:  # pragma: no cover - exercised in Kaggle runtime
            errors.append(f"direct_state_dict_load_failed={exc!r}")
            LOGGER.warning("Direct safetensors warm start failed for %s: %s", checkpoint_dir, exc)
...
def create_model_and_tokenizer(
    ...
    local_snapshot = _resolve_local_model_snapshot(resolved_name)
    if local_snapshot is not None:
        resolved_name = str(local_snapshot)
        os.environ.setdefault("HF_HUB_OFFLINE", "1")
        os.environ.setdefault("HF_DATASETS_OFFLINE", "1")
        os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
        LOGGER.info("Resolved model %s to cached local snapshot %s", model_name_or_path or model_config.base_model_name, resolved_name)
...
class MetricAwareGRPOTrainerMixin:
    ...
    def _metric_aware_save(self, checkpoint_dir: Path) -> None:
        ...
        LOGGER.info(
            "Checkpoint evaluation metrics for %s: %s",
            checkpoint_dir,
            json.dumps(eval_results.get("metrics", {}), sort_keys=True),
        )
        LOGGER.info(
            "Checkpoint subset metrics for %s: %s",
            checkpoint_dir,
            json.dumps(eval_results.get("subset_metrics", {}), sort_keys=True),
        )
